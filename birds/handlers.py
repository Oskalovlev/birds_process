...
def input_qty(hashMap, _files=None, _data=None):
    with db_session:
        p = ui_global.Record(
            ...
            name=hashMap.get("nom"),
            qty=int(hashMap.get("qty")),
        )
        commit()
...
def on_start_barcode(hashMap, _files=None, _data=None):
    ...
    table = {
        ...
        "textsize": "20",
        "columns": [
            {"name": "barcode", "header": "Barcode", "weight": "2"},
            {"name": "name", "header": "Name", "weight": "2"},
            {"name": "qty", "header": "Qty.", "weight": "1"},
        ],
    }
...
def on_start_active_cv(hashMap, _files=None, _data=None):
    ...
    for link in results:
        job = {
            "object": str(link[0]),
            "info": str(link[1]) + " </n> Остаток: <big>" + str(link[2]) + "</big>",
        }
